...
def transition(state: SystemState, event: Event) -> SystemState:
    ...
    The state intentionally exposes only generic monitor counters. Dataset adapters may
    add state values before evaluation, but they must do so deterministically and record
    that transition function in their protocol.
    """
    v = dict(state.values)
...
@dataclass(frozen=True)
class Guard:
    """Decidable monitorable guard.

    This implements a concrete subset/realisation of L5 §Rule language:
    current-event atoms, state atoms, Seen, Count, Seq, and recursive Boolean
    combinations. String operators are finite/decidable.
    """
    ...
    def eval(self, trace: Sequence[Event], state: Optional[SystemState]=None) -> bool:
        if not trace:
            return False
        e = trace[-1]
        state = state or derive_state(trace)
        ...
        if self.count_action is not None:
            h=max(1,int(self.count_horizon))
            window=trace[-h:]
            c=sum(1 for z in window
                  if (self.count_action=='*' or z.action==self.count_action)
                  and (self.count_resource is None or self.count_resource=='*' or z.resource_class==self.count_resource))
            if not self._cmp(c, self.count_op or '>=', self.count_threshold if self.count_threshold is not None else 0):
                return False

        if self.seq_actions:
            h=max(len(self.seq_actions), int(self.seq_horizon or len(self.seq_actions)))
            acts=[z.action for z in trace[-h:]]
            pos=0
            for a in acts:
...
class PolicyVersion:
    ...
    def __post_init__(self):
        ...
        if len(ids)!=len(set(ids)):
            raise ValueError('rule identifiers must be unique within one policy version')
...
def classify_from_rules(trace: Sequence[Event], rules: Sequence[Rule],
                        state: Optional[SystemState]=None, policy_version:int=0) -> EvalResult:
    app=[]; checks=0
    state=state or derive_state(trace)
    for r in rules:
        checks += 1
        if trace and r.selector.matches(trace[-1]) and r.guard.eval(trace,state):
            app.append(r)
    if not app:
        return EvalResult(DENY,GAP,(),(),checks,len(rules),policy_version)
    p=max(r.priority for r in app)
    top=[r for r in app if r.priority==p]
    eff={r.effect for r in top}
...
def reference_eval(trace: Sequence[Event], policy: PolicyVersion, enabled: Optional[Set[str]]=None,
                   state: Optional[SystemState]=None) -> EvalResult:
...
class StaticIndex:
    """Sound superset selector index. Every candidate's full guard is re-evaluated."""
    def __init__(self, policy: PolicyVersion):
        self.policy=policy
        self.buckets=defaultdict(list)
        for r in policy.rules:
            key=(r.selector.action,r.selector.resource_class,r.selector.subject_class)
            self.buckets[key].append(r)

    def candidates(self,e: Event, enabled: Optional[Set[str]]=None) -> List[Rule]:
        vals=[(e.action,'*'),(e.resource_class,'*'),(e.subject_class,'*')]
        out=[]; seen=set()
        for a,r,s in product(*vals):
            for rule in self.buckets.get((a,r,s),()):
                if rule.rid not in seen and (enabled is None or rule.rid in enabled):
                    seen.add(rule.rid); out.append(rule)
        return out

    def eval(self,trace: Sequence[Event], enabled: Optional[Set[str]]=None,
             state: Optional[SystemState]=None) -> EvalResult:
        if not trace:
            return EvalResult(DENY,GAP,(),(),0,0,self.policy.version)
        cand=self.candidates(trace[-1],enabled)
        res=classify_from_rules(trace,cand,state=state,policy_version=self.policy.version)
...
def policy_monitor_horizon(policy: PolicyVersion) -> int:
    return max([1]+[_guard_horizon(r.guard) for r in policy.rules])

class IncrementalEvaluator:
    """Version-bound streaming evaluator with deterministic state and finite monitor history."""
    def __init__(self, policy: PolicyVersion, initial_state: Optional[SystemState]=None, retention_horizon: Optional[int]=None):
        self.policy=policy
        self.index=StaticIndex(policy)
        self.retention_horizon=max(int(retention_horizon or policy_monitor_horizon(policy)), policy_monitor_horizon(policy))
        self.trace: List[Event]=[]
        self.state=initial_state or SystemState()
        self.version_history=[policy.version]

    def update_policy(self, policy: PolicyVersion):
        if policy.version <= self.policy.version:
            raise ValueError('policy updates must create a strictly newer immutable version')
        self.policy=policy
        self.index=StaticIndex(policy)
        self.retention_horizon=max(self.retention_horizon, policy_monitor_horizon(policy))
        self.version_history.append(policy.version)

    def push(self,e:Event)->EvalResult:
        self.trace.append(e)
        if len(self.trace)>self.retention_horizon:
            del self.trace[:-self.retention_horizon]
        self.state=transition(self.state,e)
        return self.index.eval(self.trace,state=self.state)
...
def _class_under_mask(trace: Sequence[Event], policy: PolicyVersion, event_positions: Sequence[int],
                      rule_ids: Sequence[str], bits: Sequence[int]) -> str:
    ne=len(event_positions)
    keep_e={event_positions[i] for i in range(ne) if bits[i]}
    keep_r={rule_ids[j] for j in range(len(rule_ids)) if bits[ne+j]}
    t=_neutralized_trace(trace,keep_e)
    return reference_eval(t,policy,enabled=keep_r,state=derive_state(t)).alert_class


def min_explain_bruteforce(trace: Sequence[Event], policy: PolicyVersion, horizon:int=4,
                          relevant_only:bool=True, max_units:int=18) -> Explanation:
    ...
    """
    factual=reference_eval(trace,policy)
    y=factual.alert_class
    ...
    if relevant_only:
        cur=trace[-1]
        rset=[r.rid for r in policy.rules if r.selector.matches(cur)]
        for rid in factual.applicable:
            if rid not in rset: rset.append(rid)
        rule_ids=rset
    else:
        rule_ids=[r.rid for r in policy.rules]
    units=[f'e:{i}' for i in event_positions]+[f'r:{r}' for r in rule_ids]
    n=len(units); checks=0
    if n>max_units:
        raise RuntimeError(f'bounded brute-force explainer unit limit exceeded: {n}>{max_units}')
    ...
    def suff(core_idx:Set[int]):
        ...
        for vals in product([0,1], repeat=len(free)):
            ...
            checks += 1
            if _class_under_mask(trace,policy,event_positions,rule_ids,bits)!=y:
                return False,bits
        return True,None
    ...
    ep=tuple(event_positions[i] for i in range(len(event_positions)) if i in core)
    off=len(event_positions)
    rr=tuple(rule_ids[j] for j in range(len(rule_ids)) if off+j in core)
    return Explanation(y,ep,rr,witnesses,n,checks,policy.version)


def exact_sufficiency_of_selected(trace: Sequence[Event], policy: PolicyVersion, selected_units:Set[str],
                                  horizon:int=4, max_units:int=18):
    ...
    cur=trace[-1]
    rule_ids=[r.rid for r in policy.rules if r.selector.matches(cur)]
    units=[f'e:{i}' for i in event_positions]+[f'r:{r}' for r in rule_ids]
    core={i for i,u in enumerate(units) if u in selected_units}
    n=len(units)
    ...
    for r in rules:
        ...
        for tr in domain_traces:
            ...
            if r.selector.matches(tr[-1]) and r.guard.eval(tr,st):
                applicable_cases.append(tr)
                res=reference_eval(tr,policy,state=st)
        ...
        if all(reference_eval(tr,policy).alert_class==reference_eval(tr,p2).alert_class for tr in domain_traces if tr):
            redundant.append(r.rid)
...
def mutate_conflict(policy:PolicyVersion, target_rid:str, new_version:int)->PolicyVersion:
    r=next(x for x in policy.rules if x.rid==target_rid)
    opp=DENY if r.effect==ALLOW else ALLOW
    nr=Rule(rid=f'M_CONFLICT_{target_rid}',priority=r.priority,effect=opp,selector=r.selector,guard=r.guard)
    return PolicyVersion(policy.pid,new_version,policy.rules+(nr,))

def mutate_shadowing(policy:PolicyVersion,target_rid:str,new_version:int)->PolicyVersion:
    r=next(x for x in policy.rules if x.rid==target_rid)
    nr=Rule(rid=f'M_SHADOW_{target_rid}',priority=r.priority-100,effect=r.effect,selector=r.selector,guard=r.guard)
    return PolicyVersion(policy.pid,new_version,policy.rules+(nr,))

def mutate_redundancy(policy:PolicyVersion,target_rid:str,new_version:int)->PolicyVersion:
    r=next(x for x in policy.rules if x.rid==target_rid)
    nr=Rule(rid=f'M_REDUNDANT_{target_rid}',priority=r.priority,effect=r.effect,selector=r.selector,guard=r.guard)
    return PolicyVersion(policy.pid,new_version,policy.rules+(nr,))

def mutate_gap(policy:PolicyVersion,target_rids:Sequence[str],new_version:int)->PolicyVersion:
    rm=set(target_rids)
    return PolicyVersion(policy.pid,new_version,tuple(r for r in policy.rules if r.rid not in rm))
